...
import { FaHtml5, FaCss3Alt, FaJs, FaReact, FaGithub, FaExternalLinkAlt, FaDocker, FaAws, FaBootstrap, FaNodeJs, FaGitAlt } from 'react-icons/fa';
...
const Projects = () => {
    ...
    const getTechIcon = (tech) => {
        switch (tech.toLowerCase().replace('.', '').replace(' ', '')) {
            ...
            case 'supabase': return <SiSupabase className="text-emerald-500" />;
            case 'vercel': return <SiVercel className="text-black dark:text-white" />;
            case 'framermotion': return <SiFramer className="text-pink-500" />;
            case 'git': return <FaGitAlt className="text-red-500" />;
            default: return <span className="text-xs font-bold text-gray-500">{tech.slice(0,2)}</span>;
        }
    };

    return (
        <section id="projects" className="py-24 bg-bg-light dark:bg-bg-dark text-text-light dark:text-text-dark font-poppins transition-colors duration-300 relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {PROJECTS.map((project, index) => (
                        <motion.div
                            ...
                            className="bg-white dark:bg-[#1e1c1c] rounded-lg p-6 text-center transition-all duration-300 hover:bg-primary hover:text-white group cursor-default shadow-lg hover:shadow-primary/40 dark:shadow-none border border-gray-100 dark:border-gray-800 flex flex-col h-full"
                        >
